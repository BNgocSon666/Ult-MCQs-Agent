...
function ResultQuestion({ question, index }) {
  ...
  return (
    <div className={`result-question-card ${is_correct ? 'correct' : 'incorrect'}`}>
      ...
      <div className="result-footer">
        {is_correct ? (
          <span>Bạn đã chọn đúng: <strong>{selected_option}</strong></span>
        ) : (
          <span>
            Bạn đã chọn: <strong>{selected_option || '(Bỏ qua)'}</strong> | 
            Đáp án đúng: <strong>{answer_letter}</strong>
          </span>
        )}
      </div>
    </div>
...
function ExamResultsPage() {
  ...
  if (isLoading) return <div className="loading-container">Đang tải kết quả...</div>;
